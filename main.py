...
# Spin symbols count and their values
symbol_count = {
    "A": 2,
    "B": 4,
    "C": 6,
...
def check_winnings(columns, lines, bet, values):
    winnings = 0
    winning_lines = []
    # Check column-wise for all equal symbols
...
def get_slot_machine_spin(rows, cols, symbols):
    # Generate all symbols according to their count in dictionary
    all_symbols = []
    for symbol, symbol_count in symbols.items():
        for _ in range(symbol_count):
            all_symbols.append(symbol)
    
    # Generate random symbols to put in each row of all columns
    columns = []
    for _ in range(cols):
        column = []
        # shallow copy of a list (using splice operator)
